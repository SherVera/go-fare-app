...
export default function DriverDashboard() {
  ...
  const [vehicleInfo, setVehicleInfo] = useState('Encava ENT-610 (Placa: XY987ZT)');
  const [todayTripsCount, setTodayTripsCount] = useState(42);
  const [todayEarnings, setTodayEarnings] = useState(630.00);
    ...
    try {
      ...
      if (user) {
        ...
        try {
          ...
          setDriverName(backendUser.displayName || `${backendUser.firstName || ''} ${backendUser.lastName || ''}`.trim() || 'Conductor');
        } catch {
      ...
      const validatedListStr = await AsyncStorage.getItem('mock_validated_tickets');
      const validatedList = validatedListStr ? JSON.parse(validatedListStr) : [];
      
      // Sumar al total del día
      const localCount = validatedList.length;
      const localEarnings = validatedList.reduce((sum: number, tx: any) => sum + (tx.fare || 15.00), 0);

      setTodayTripsCount(42 + localCount);
      setTodayEarnings(630.00 + localEarnings);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.greetingSection}>
          ...
          <Text style={styles.greetingSub}>Controla tu turno de trabajo desde aquí.</Text>
        </View>

        {/* Card Estado de Servicio */}
        <View style={[styles.statusCard, isEnServicio ? styles.statusCardActive : styles.statusCardInactive]}>
          <View style={styles.statusInfoRow}>
            <View style={[styles.statusIconWrapper, isEnServicio ? styles.iconActive : styles.iconInactive]}>
              <Ionicons name="bus" size={24} color="#FFFFFF" />
            </View>
            <View style={styles.statusTexts}>
              <Text style={styles.statusPillLabel}>ESTADO DEL TURNO</Text>
              <Text style={styles.statusPillValue}>{isEnServicio ? 'En Servicio / Disponible' : 'Fuera de Servicio'}</Text>
            </View>
            <Switch
              value={isEnServicio}
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.vehicleRow}>
            <Ionicons name="car-sport" size={24} color={tokens.colors.primary} style={{ marginRight: 12 }} />
        ...
        >
          <Ionicons name="map-outline" size={20} color="#8594AB" style={{ marginRight: 12 }} />
          <Text style={styles.dropdownBtnText}>{selectedRoute.name}</Text>
          <Ionicons name={showRouteDropdown ? 'chevron-up' : 'chevron-down'} size={20} color="#8594AB" />
        </Pressable>
        ...
        {showRouteDropdown && (
          <View style={styles.dropdownContainer}>
            {ROUTE_OPTIONS.map((route) => (
              <Pressable
                key={route.id}
                style={[styles.dropdownItem, selectedRoute.id === route.id && styles.dropdownItemActive]}
                onPress={() => handleSelectRoute(route)}
              >
                <Text style={[styles.dropdownItemText, selectedRoute.id === route.id && styles.dropdownItemTextActive]}>
                  {route.name}
                </Text>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBox}>
            <View style={[styles.statIconContainer, { backgroundColor: '#DBEAFE' }]}>
              <Ionicons name="ticket-outline" size={20} color={tokens.colors.primary} />
            </View>
